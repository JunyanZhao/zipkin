...
public abstract class ITSpanStore {
  ...
  @Test public void getTraces_multipleAnnotationsBecomeAndFilter() throws IOException {
    ...
    assertThat(sortTraces(store().getTraces(
      ...
      asList(barAndFooAndBazAndQux), asList(fooAndBazAndQux), asList(barAndFoo), asList(foo)
    );

    assertThat(sortTraces(store().getTraces(
      requestBuilder().serviceName("frontend").parseAnnotationQuery("foo and bar").build()
    ).execute())).containsExactly(
      asList(barAndFooAndBazAndQux), asList(barAndFoo)
    );
    ...
    assertThat(sortTraces(store().getTraces(
      ...
      asList(barAndFooAndBazAndQux), asList(fooAndBazAndQux)
    );
  }
  ...
  public void getTraces_differentiateOnServiceName() throws IOException {
    ...
    assertThat(store().getTrace(trace2.traceId()).execute())
